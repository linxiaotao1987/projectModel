 ...
 */
(function() {
    window.LUploader = function(el, params,callback) {
        var _self = this;
        _self.trigger=el;
        _self.params = {
            accept: 'image/*',
        ...
        _self.trigger.addEventListener('change', function() {
            ...
            files.forEach(function(file, i) {
                ...
                if (!_self.params.multiple) { //假如是单个上传
                    if (_self.old_li) {
                        console.log(LUploaderList);
                        console.log(_self.old_li);
                        console.log(_self.old_li.parentNode);
                        // LUploaderList.removeChild(_self.old_li);
                        LUploaderList.innerHTML = '';
                    } else {
                        _self.old_li = li;
                    }
                }
    ...
    LUploader.prototype.upload = function(url, obj, basestr, type, li) {
        ...
        xhr.onload = function(e) {
            ...
            if ((xhr.status >= 200 && xhr.status < 300) || xhr.status === 0) {
                ...
                span.innerHTML = text;
                _self.callback && _self.callback('上传成功',data.data);
            }
            else {
                span.innerHTML = '上传失败';
                _self.callback && _self.callback('上传失败');
            }
        }
